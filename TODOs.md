...
- ~~**Input actions could never fire from the mouse wheel, and dropped fast taps**~~ — FIXED. `IsTriggerActive` tested only the *held* state (`Input::IsKeyDown` / `IsMouseControlDown`). A wheel notch has no held state at all — `PulseMouseControl` sets both edge latches and never touches the level array — so every `Mouse/WheelUp` / `Mouse/WheelDown` binding was silently dead, despite the editor's trigger picker offering them from `MouseControls.inl`. The same gap swallowed any press+release delivered inside one `glfwPollEvents` batch, which is exactly what the press/release latches exist to catch. Both device branches now test `Down || Pressed`. Guarded by `Input::WheelBoundActionFiresForOneFrame`, `Input::ActionCatchesATapInsideOnePoll`.
- ~~**`InputActionManager`'s by-name mutators indexed with `-1`**~~ — FIXED alongside the query functions. `GetActionIndex` returns `-1` for an unknown name; `RemoveAction(string_view)` fed that to an `EB_CORE_ASSERT` (a `__debugbreak()` in Debug), and `AddTrigger(string_view, ...)` / `RemoveTrigger(string_view, int)` fed it straight into `m_Actions[-1]` with no check at all. All three now return early, matching the guard the query functions grew. `AddTrigger(int, ...)` also gained the bounds assert its siblings already had. Guarded by `Input::MissingActionMutatorsAreNoOps`, `Input::MissingActionQueriesReturnFalseInsteadOfIndexingOutOfRange`.
- ~~**`Input.IsMouseControlPressed` was bound to the held state in Lua**~~ — FIXED. The new mouse bindings wired both `IsMouseControlDown` and `IsMouseControlPressed` to `Input::IsMouseControlDown`, which made the second name useless and left scripts with no way to read the scroll wheel at all (the wheel is only ever *pressed*, never *down*). Now points at `Input::IsMouseControlPressed`. The older `IsKeyPressed` / `IsMouseButtonPressed` spellings stay aliased to the held state on purpose — game scripts have always read them that way — and are pinned by `Input::LuaIsKeyPressedIsTheHeldStateNotTheEdge`.
...
- ~~**Transform decompose produces NaNs on negative scaling**~~ — STALE, and the line reference no longer pointed at any decompose call. `MathDetail::DecomposeTransform` ([Math.h#L140](Ember/src/Ember/Math/Math.h#L140)) already handles it: it checks the basis determinant, negates `scale.x` and `basisX` when the matrix is mirrored, guards near-zero scale, and returns `bool` for failure. **The remaining mirroring risk is downstream, not in the math** — a negative scale reverses triangle winding, and there is no `glFrontFace` call anywhere in the renderer (only a global `RenderAction::CullFace`), so a mirrored kit piece will likely render inside-out under backface culling. Verify by mirroring one wall piece; if inside-out, flip the cull face for negative-determinant transforms in `DeferredGeometryRenderPass`.
...
- ~~**`static_cast<bool>(entity)` selected `operator EntityID()` and inverted**~~ — FIXED. `Entity` declared a non-explicit, **non-const** `operator EntityID()` alongside `explicit operator bool() const`; the non-const conversion bound the implicit object argument better, so a boolean context chose the integer conversion. A valid entity with handle `0` read as `false` and an invalid entity read as `true`. The fix is one word — making `operator EntityID()` **const** breaks the tie on the object argument, after which `bool -> bool` (identity) beats `uint32_t -> bool` (boolean conversion) and `operator bool` wins. Making it `explicit` would also have worked but would have broken every `entity != Constants::Entities::InvalidEntityID` comparison in the engine; the `const` fix required no call-site changes. Guarded by `Core::EntityBooleanConversionPicksOperatorBool`, which pins both directions plus the handle-`0` case.
...
- ~~**Prefab instances never updated when the prefab changed**~~ — DONE, with limits worth knowing. `Scene::RefreshPrefabInstances` rebuilds each placed instance from the prefab's current definition. **Survives per instance:** position/rotation/scale, name, parent, sibling index, the instance's own UUID (so inbound references keep resolving), and `ScriptComponent::UserPropertyOverrides` — matched by name path down the subtree, since child UUIDs are regenerated. **Does not survive:** swapped materials or sprites, added components, any other per-instance edit. Offered via a confirmation naming the instance count rather than firing silently on save, and pushed onto the *scene tab's* undo stack (not the active tab's, which is the prefab being edited).
  - **Fixed along the way:** `Scene::CreatePrefab` serialized the entity **before** attaching its `PrefabComponent`, so the saved YAML had no prefab link and every instance placed from it was an orphan no refresh could find. The prefab's UUID only exists after the first save, so the fix is a second serialize pass. Instances placed before this fix are still orphans and must be re-placed once to become linked.
  - **Known limits:** renaming a child inside an instance breaks the name-path match, so that child's script overrides are lost on refresh. Duplicate sibling names within a prefab resolve to the first match. Full per-property override tracking (Unity-style, with "modified" markers) remains the proper long-term answer.

- ~~**No undo/redo anywhere**~~ — DONE (snapshot transactions, not a command pattern). `EntitySetSnapshot` lives engine-side in [Ember/src/Ember/Scene/EntitySnapshot.h](Ember/src/Ember/Scene/EntitySnapshot.h) so `Ember-Test` can cover it, and reuses the prefab serializer — which already round-trips every component and repairs parent/child links through a UUID remap. A command class per mutation would have meant touching ~36 ComponentUIs and leaving every future component silently without undo. Editor side is `Ember-Forge/src/Undo/`: `UndoStack` (per viewport tab, bounded by entry count and bytes), `ScopedEntityEdit` (RAII bracket for structural actions), and `SelectionEditTracker`, which re-baselines the selection every idle frame and diffs on the falling edge of `ImGui::IsAnyItemActive() || ImGuizmo::IsUsing()` — so a whole drag or typing session collapses into one entry with **zero changes to `PropertyGrid` or any component UI**. Ctrl+Z / Ctrl+Y / Ctrl+Shift+Z, plus an Edit menu that names the action. Guarded by seven `Scene::Snapshot*` tests.
  - **Still to hook:** rename is covered by the tracker only while the field has focus; scene-level operations (save, scene load) are deliberately outside history. Prefab-tab and scene-tab stacks are separate, and closing a tab discards its history.
  - **Known cost:** the idle baseline is one `SerializeEntityNode` per selected entity per frame (~1-3KB of YAML each). Fine for typical selections, worth a Profile capture if a large multi-selection ever feels sluggish — `SelectionEditTracker::OnFrameEnd` already carries an `EB_PROFILE_FUNCTION`.
...
- ~~**Spawn new entities at the cursor rather than at the origin**~~ — DONE. `EditorLayer::GetSpawnPosition()` reads world position from the deferred G-Buffer's `PositionAO` attachment under the cursor (better than a physics ray: it works on geometry that has no collider yet), snaps it to the grid increment when snapping is on, and falls back to the camera focal point on a miss. Wired into `CreateEntityFromPrefab`, `CreateEntityFromModel`, and `SceneHierarchyPanel::CreateEntity` — the single convergence point for every preset menu item. UI entities and non-root entities are skipped, since their `Position` is not a world position. Exposed to panels as `EditorContext::SpawnPosition`.
