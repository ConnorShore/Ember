...
float NormalDistributionTrowbridgeReitxGGX(vec3 N, vec3 H, float roughness)
{
	...
	float denomBulk = (nDotHSqr * (aSqr - 1)) + 1;
...
{	
	...
	for (int i = 0; i < MAX_LIGHTS; i++)
	{
		...
		float D = NormalDistributionTrowbridgeReitxGGX(N, H, u_Roughness);
		float G = max(GeometrySchlickGGXSub(N, V, u_Roughness), 0.0) * max(GeometrySchlickGGXSub(N, L, u_Roughness), 0.0);
