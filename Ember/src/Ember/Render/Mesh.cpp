...
namespace Ember {
	...
	Mesh::Mesh(UUID uuid, const std::string& name, const std::vector<MeshVertex>& vertices, const std::vector<unsigned int>& indices)
		: Asset(uuid, name, "", GetStaticType())
	{
		auto vbo = VertexBuffer::Create(&vertices[0], sizeof(MeshVertex) * vertices.size(), {
			{ ShaderDataType::Float3, "v_Position" },
			{ ShaderDataType::Float3, "v_Normal" },
			{ ShaderDataType::Float2, "v_TextureCoord"},
			{ ShaderDataType::Float3, "v_Tangent" },
