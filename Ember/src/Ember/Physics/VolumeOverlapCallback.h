	...
	public:
		...
		virtual void onOverlap(reactphysics3d::OverlapCallback::CallbackData& callbackData) override
		{
			...
			for (uint32_t i = 0; i < overlapCount; i++)
			{
				...
				if (!HasEntityUserData(collider->getUserData()) && !HasEntityUserData(collidedBody->getUserData()))
					continue;
				...
				data.CollidedEntity = HasEntityUserData(collider->getUserData())
					? DecodeEntityUserData(collider->getUserData())
					: DecodeEntityUserData(collidedBody->getUserData());
